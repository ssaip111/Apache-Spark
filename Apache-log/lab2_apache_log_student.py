...
endpointCountPairTuple = not200.map(lambda a: (a.endpoint,1))
...
hosts = access_logs.map(lambda a: (a.host,1))
...
dayToHostPairTuple = access_logs.map(lambda a: (a.date_time.day,a.host)).distinct()
dayGroupedHosts = dayToHostPairTuple.groupByKey().mapValues(len)
#print dayGroupedHosts.collect()

dayHostCount = dayGroupedHosts.sortByKey(True,1)
...
dayAndHostTuple = access_logs.map(lambda a: (a.date_time.day,a.host))
...
avgDailyReqPerHost = (sortedByDay
                     .map(lambda (x,y): (x,y[0]/y[1])).cache())
...
# 'Found %d 404 URLs' % badRecords.count()
...
badEndpoints = badRecords.map(lambda x: x[3])

badUniqueEndpoints = badEndpoints.distinct()
print badUniqueEndpoints
...
badEndpointsCountPairTuple = badRecords.map(lambda x: (x[3],1))

badEndpointsSum = badEndpointsCountPairTuple.groupByKey().mapValues(len)
#print badEndpointsSum.collect()                                                       

badEndpointsTop20 = badEndpointsSum.sortBy(lambda x: x[1], ascending = False).take(20)
...
errHostsSum = errHostsCountPairTuple.groupByKey().mapValues(len)

errHostsTop25 = errHostsSum.sortBy(lambda x: x[1], ascending = False).take(25)
...
errDateSum = errDateCountPairTuple.groupByKey().mapValues(len)
...
topErrDate = errDateSorted.sortBy(lambda x: x[1], ascending = False).take(5)
