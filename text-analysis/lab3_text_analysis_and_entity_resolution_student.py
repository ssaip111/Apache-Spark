...
def simpleTokenize(string):
    ...
    """
    return filter(lambda x: len(x)>0, re.split(r'\W+',string.lower()))
...
def tokenize(string):
    ...
    """if string == ' ':
        ...
        mystr = re.sub('[^A-Za-z0-9\s\_\.\/]+', "", string).lower().strip('.').strip(' ')
        splitstr = re.compile(split_regex).split(mystr)
        res = []
        for s in splitstr:
    ...
    return res """
    lst = []
    for x in simpleTokenize(string):
        if x not in list(stopwords):
            lst.append(x)
    return lst
...
def findBiggestRecord(vendorRDD):
    ...
    """
    vendorsame = vendorRDD.reduceByKey(lambda a: a+b )   
    finaloutput = (vendorsame.sortBy(lambda x: len(x[1]),ascending=False)).collect()
    return finaloutput
...
def tf(tokens):
    ...
    maplist = rdd.map(lambda x: (x,1))
    countlist = maplist.countByKey().items()
    cnt = rdd.count()
    TFvalues = sc.parallelize(countlist).map(lambda a: (a[0], (a[1]/float(cnt)))).collect()     
    return dict((TFvalues))
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.map(lambda a: (a[1],1)).reduceByKey(lambda a,b : a+b)
    tokenSumPairTuple = tokenCountPairTuple.sortBy(lambda x: x[1],ascending = False)
    return  (tokenSumPairTuple.map(lambda (a,b): (a,N/float(b))))
...
def dotprod(a, b):
    ...
    """
    the_sum = {}
    the_sum = { b[k] * a[k] for k, v in a.items() if k in b}
    return sum(list(the_sum))
...
def norm(a):
    ...
    """
    the_sum = {a[k] * a[k] for k,v in a.items()}
       
    return sum(list(the_sum)) ** (0.5)
...
def cossim(a, b):
    ...
    """
    the_sum = dotprod(a,b)/ (norm(a)*norm(b))
    return the_sum
...
similarities = (crossSmall
                ...
                .cache())
'''similarities = computeSimilarity([('http://www.google.com/base/feeds/snippets/11448761432933644608',
