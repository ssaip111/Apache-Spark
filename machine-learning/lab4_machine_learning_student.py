...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    sumlst = (x[0], (len(x[1]), sum(x[1])/float(len(x[1]))))
        
        
    
    return sumlst #<FILL IN>
...
movieIDsWithRatingsRDD = (ratingsRDD.map(lambda (x,y,z):(y,z)).groupByKey())
...
movieNameWithAvgRatingsRDD = (moviesRDD.join(movieIDsWithAvgRatingsRDD)
                              .map(lambda ((mId,(mname,(numRatings,avgRatings)))): (avgRatings, mname,numRatings))
                              .sortBy(lambda x: (x[0],x[1])).cache())
...
print 'Movies with highest ratings: %s' % movieLimitedAndSortedByRatingRDD.take(100)
...
def computeError(predictedRDD, actualRDD):
    ...
    totalError = squaredErrorsRDD.map(lambda (x,y): (x,(y[0]-y[1])**2))
   
    # Count the number of entries for which you computed the total squared error
    numRatings = squaredErrorsRDD.count()
        
    s = 0
    for x in list(totalError.collect()):
        s += x[1]
        
        
   
    # Using the total squared error and the number of entries, compute the RSME
    return (s/ float(numRatings))**(0.5)
...
validationForPredictRDD = validationRDD.map(lambda(uid, mid, r): (uid, mid))
...
# TODO: Replace <FILL IN> with appropriate code
myModel =  ALS.train(trainingRDD, bestRank, seed=5L, iterations=5,
                      lambda_= 0.1)
...
myRatedMovies = [
     (myUserID, 260, 5), (myUserID, 34, 4.3), (myUserID, 245, 2.5), (myUserID, 123, 3.5),
     (myUserID, 2340,2.5) , (myUserID, 111, 4), (myUserID, 345, 2.1), (myUserID, 603, 4.756),
     (myUserID, 132, 1) , (myUserID, 555, 3)
...
#print myUnratedMoviesRDD.collect()
