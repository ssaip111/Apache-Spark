...
parsedTrainData.collect()
parsedValData.collect()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    temp = labelsAndPreds.map(lambda x: squaredError(x[0],x[1])).mean()
    return temp** .5
...
def getLabeledPrediction(weights, observation):
    ...
    """
    #return (DenseVector(weights), Laobservation)
    #w = DenseVector(weights)
    #o = LabeledPoint(observation)
...
#print predictionExample.collect()
...
#print weightsLR0
#print errorTrainLR0
#print parsedValData.take(3)

labelsAndPreds = parsedValData.map(lambda lp:(lp.features, weightsLR0))
rmseValLR0 = calcRMSE(labelsAndPreds)
print calcRMSE(labelsAndPreds)
...
firstModel = LinearRegressionWithSGD.train(data= parsedTrainData, iterations=numIters, step=alpha,
    miniBatchFraction=miniBatchFrac, regParam=reg, regType=regType,intercept=useIntercept)
...
def twoWayInteractions(lp):
    ...
    """
    lst = lp.features
    temp = itertools.product(lp.features,lp.features)
    temp =[x*y for (x,y) in list(temp)]
    lst =  list(lst) + (list(temp))
    return (LabeledPoint(lp.label,lst))

print twoWayInteractions(LabeledPoint(1.0, [1, 2, 3]))
